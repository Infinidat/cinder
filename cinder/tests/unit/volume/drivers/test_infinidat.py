...
class InfiniboxDriverTestCase(InfiniboxDriverTestCaseBase):
    ...
    @ddt.unpack
    def test_system_compression(self, system_compression, config_compression):
        self.override_config('infinidat_use_compression', config_compression)
        self._system.compat.has_compression.return_value = system_compression
        if config_compression and not system_compression:
            self.assertRaises(exception.VolumeDriverException,
                              self.driver.do_setup, None)
        else:
    ...
    @mock.patch('cinder.volume.volume_types.get_volume_type_qos_specs')
    @mock.patch('cinder.volume.utils.group_get_by_id')
    ...
    def test_create_cloned_volume(self, *mocks):
        ...
        self.assertEqual(1, self._mock_volume.create_snapshot.call_count)
        self.assertEqual(1, self._mock_volume.safe_delete.call_count)
    ...
    @mock.patch('cinder.volume.volume_types.get_volume_type_qos_specs')
    @mock.patch('cinder.volume.utils.check_already_managed_volume',
                return_value=False)
